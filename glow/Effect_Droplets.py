...
class Droplet(object):
    def __init__(self, args):
        ...
        self.valid = True
        self.values = list(
            scipy.signal.gaussian(self.radius * 2 + 1, self.radius / 2.0)
        )
        self.iteration = 0
...
class Droplets(object):
    ...
    def iterate(self):
        ...
        if len(self.droplets) < self.max and random.random() < self.probability:
...
def create_parser():
    ...
    parser.add_argument(
        ...
        default=3,
        type=arg_positive(),
        help="maximum number of droplets" " (default: %(default)s)",
    )
    parser.add_argument(
        "--probability",
        default=0.5,
        type=arg_positive(float),
        help="probability of creating a new droplet" " (default: %(default)s)",
    )
    return parser
