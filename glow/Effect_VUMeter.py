...
class VUMeter(object):
    def __init__(self, args):
        ...
        hue_end = args.hueend
        hue_values = [
            hue_start + ((hue_end - hue_start) / float(self.nled)) * x
            for x in range(self.nled)
        ]
        self.texture = list(
            chain.from_iterable((hue, 1.0, 1.0) for hue in hue_values)
        )
        self.begin = 0
        self.end = 41
    ...
    def iterate(self):
        LevelRPercent = (self.source.LevelR - self.begin) / float(
            self.end - self.begin
        )
